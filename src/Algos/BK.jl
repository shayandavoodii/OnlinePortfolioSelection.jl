"""
    bk(data::Matrix{T}, K::S, L::S, c) where {T<:Float64, S<:Int}
...
function bk(data::Matrix{T}, K::S, L::S, c) where {T<:Float64, S<:Int}
  -1≤c≤1 || DomainError("c must be between -1 and 1 (-1 ≤ c ≤ 1)") |> throw
  nstocks, ndays= size(data)
  ...
  for t = 1:ndays
    ...
    daily_ret[t, 1] = sum(data[:, t].*day_weight)
    day_weight_o = day_weight .* data[:, t]./daily_ret[t]
    exp_ret[L+1, 1] = exp_ret[L+1, 1]*sum(data[:, t].*exp_w[:, K*L+1])
    for l=1:L
      for k=1:K
        exp_ret[l, k] = exp_ret[l, k]*sum(data[:, t].*exp_w[:, (k-1)*L+l])
      end
    end
  end
